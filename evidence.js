async function loadEvidenceVault() {
  ...
  try {
    ...
    gridContainer.innerHTML = evidenceList.map(item => {
      ...
      return `
        <article class="card" data-type="${item.type}" data-status="${item.status}" style="border: 1px solid #333; padding: 1rem; border-radius: 8px; background: #181818; text-align: left;">
          <div style="display: flex; justify-content: space-between; align-items: center;">
            <span class="badge ${statusClass}" style="padding: 0.25rem 0.5rem; border-radius: 4px; font-size: 0.75rem; text-transform: uppercase; font-weight: bold; background: #2a2a2a; color: #00ffcc;">${statusLabel}</span>
